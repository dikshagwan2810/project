...
provider "azurerm" {
   version = "=2.10.0"
   subscription_id = "52ec8630-a802-4b70-8376-1c7a34ac9a6f"
...
resource "time_sleep" "wait_60_seconds_for_ACR" {
  depends_on = [azurerm_role_assignment.acrpull_role]
...
resource "time_sleep" "wait_60_seconds_for_aks" {
  depends_on = [azurerm_role_assignment.snetakscontributerole]
